...
import { discover, resolve_token } from "../env/registry.ts";
...
export async function startAnvil(): Promise<AnvilInstance> {
  ...
  const contractRun = (Deno.env.get("CONTRACT_RUN") ?? "v10c") as RunVersion;
  ...
  const fundToken = async (
    ...
    try {
      const reg = await discover(provider, contractRun);
      const tokenAddr = resolve_token(symbol.toUpperCase(), reg);
