...
Deno.test("parse [--pool={none,1}]", () => {
  const defs = parse([]);
  assertEquals(defs.pool, undefined);
  assertEquals(defs.rest, []);
  const args = parse(["--pool=1"]);
  assertEquals(args.pool, 1);
  assertEquals(args.rest, []);
});
Deno.test("parse [--oracle={none,UNUS}]", () => {
  const defs = parse([]);
  assertEquals(defs.oracle, undefined);
  assertEquals(defs.rest, []);
  const args = parse(["--oracle=UNUS"]);
  assertEquals(args.oracle, "UNUS");
  assertEquals(args.rest, []);
});
