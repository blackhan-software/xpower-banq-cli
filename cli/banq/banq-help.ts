...
export default function help(
  ...
  if (args.rest.length > 0) {
    ...
    if (args.rest[0] === "track-position" || args.rest[0] === "tp") {
      return cmd_token_tp(
        args,
        "track-position",
        "Track position balances. 📊",
      );
    }
    if (
      args.rest[0] === "cap-limit" || args.rest[0] === "cap" ||
      args.rest[0] === "cl"
    ) {
      return cmd_cap_limit(args);
    }
...
function cmd_token_tp(
  args: BanqArgs,
  command: string,
  slug: string,
) {
  cmd_token_head_tp(command, slug);
  pool_options();
  track_position_options();
  wallet_options();
  gas_options();
  general_options();
  other_options();
  option_defaults();
  pool_examples_tp(args);
}
function cmd_token_head_tp(
  command: string,
  slug: string,
) {
  console.log(`XPower Banq: ${slug}`);
...
function pool_commands() {
  ...
  console.log(
    "  " + [
      ...
      "",
      "cl|cap-limit:\t\t🧢 Set or adjust cap limits",
      ...
      "tp|track-position:\t📊 Tracks position balances",
    ].join("\n  "),
  );
}
...
function general_options() {
  ...
  console.log(
    "  " + [
      "-u|--provider-url $PROVIDER_URL: [comma-sep]",
      "\t\t\t🌐 RPC provider(s) (comma-sep, defaults built-in)",
      "-V|--contract-run $CONTRACT_RUN: [default=none]",
      "\t\t\t📦 Run version (v10b, v10c, v11a)",
...
function option_defaults() {
  ...
  console.log(
    "  " + [
      "PROVIDER_URL\t\t🌐 11 built-in providers (comma-sep, WS-pref)",
  ...
  console.log(
    "  " + [
      !args //
        ...
        : "",
      !args || args.rest[0] === "tp" || args.rest[0] === "track-position"
        ? "banq tp APOW -M supply  -Y [--pool=P000] # 📊"
        : "",
      !args || args.rest[0] === "tp" || args.rest[0] === "track-position"
        ? "banq tp XPOW -M borrow  -Y [--pool=P000] # 📊"
        : "",
      !args || args.rest[0] === "tp" || args.rest[0] === "track-position"
        ? "banq tp APOW -M supply  -w               # 👀 live"
        : "",
      !args || args.rest[0] === "tp" || args.rest[0] === "track-position"
        ? "banq tp APOW -M supply  -w=2048          # 👀 last 2048 blocks"
        : "",
      !args || args.rest[0] === "tp" || args.rest[0] === "track-position"
        ? "banq tp APOW -M supply  -w=2048@3        # 👀 chunk 3 back"
        : "",
      !args || args.rest[0] === "tp" || args.rest[0] === "track-position"
        ? "banq tp APOW -M supply  -w=2048@all      # 👀 all history"
        : "",
    ].filter((s) => s).join("\n  "),
  );
}
...
///
/// TRACK-POSITION OPTIONs
///
function track_position_options() {
  console.log(
    "\n%cTRACK-POSITION OPTIONs",
    "font-weight: bold",
  );
  console.log(
    "  " + [
...
///
/// CAP-LIMIT COMMAND & OPTIONs & EXAMPLEs
///
function cmd_cap_limit(args: BanqArgs) {
  console.log("XPower Banq: Set or adjust cap limits. 🧢");
  usage_title();
  console.log(
    "  banq cap-limit $TOKEN $VALUE --mode=supply [--options]",
  );
  console.log(
    "  banq cap-limit $TOKEN $VALUE --mode=borrow [--options]",
  );
  pool_options();
  cap_limit_options();
  wallet_options();
  ...
  other_options();
  option_defaults();
  cap_limit_examples(args);
}
function cap_limit_options() {
  console.log(
    "\n%cCAP-LIMIT OPTIONs",
    "font-weight: bold",
  );
  console.log(
    "  " + [
      "-M|--mode $MODE:\t🔄 Cap mode (supply, borrow)",
      "--inc|--increase:\t📈 Increase cap by value (relative)",
      "--dec|--decrease:\t📉 Decrease cap by value (relative)",
    ].join("\n  "),
  );
}
function cap_limit_examples(args?: BanqArgs) {
  console.log(
    "\n%cEXAMPLEs",
    "font-weight: bold",
  );
  console.log(
    "  " + [
      !args ||
        args.rest[0] === "cl" || args.rest[0] === "cap" ||
        args.rest[0] === "cap-limit"
        ? "banq cap-limit APOW 1000 -M supply -Y    # 🧢 set abs. supply cap"
        : "",
      !args ||
        args.rest[0] === "cl" || args.rest[0] === "cap" ||
        args.rest[0] === "cap-limit"
        ? "banq cap-limit XPOW    0 -M borrow -Y    # 🧢 clr abs. borrow cap"
        : "",
      !args ||
        args.rest[0] === "cl" || args.rest[0] === "cap" ||
        args.rest[0] === "cap-limit"
        ? "banq cap APOW 500 -M supply --inc  -Y    # ➕ increase supply cap"
        : "",
      !args ||
        args.rest[0] === "cl" || args.rest[0] === "cap" ||
        args.rest[0] === "cap-limit"
        ? "banq cl  XPOW 200 -M borrow --dec  -Y    # ➖ decrease borrow cap"
        : "",
    ].filter((s) => s).join("\n  "),
  );
}
