...
/** Original env values captured before mock overrides, restored by teardown. */
const SAVED_ENV = new Map<string, string | undefined>();
function save_env(key: string): void {
  if (!SAVED_ENV.has(key)) {
    SAVED_ENV.set(key, Deno.env.get(key));
  }
}
function restore_env(key: string): void {
  const prev = SAVED_ENV.get(key);
  if (prev === undefined) {
    Deno.env.delete(key);
  } else {
    Deno.env.set(key, prev);
  }
  SAVED_ENV.delete(key);
}
...
function setup(
  ...
  cfg.pools.forEach((pool, i) => {
    const v10c = `${pool_name(i)}_ADDRESS_v10c`;
    const v11a = `${pool_name(i)}_ADDRESS_v11a`;
    save_env(v10c);
    save_env(v11a);
    Deno.env.set(v10c, pool.address);
    Deno.env.set(v11a, pool.address);
  });
  const caps = random();
  save_env("CAPS_ADDRESS_v10c");
...
function teardown(cfg: MockConfig): void {
  cfg.pools.forEach((_, i) => {
    restore_env(`${pool_name(i)}_ADDRESS_v10c`);
    restore_env(`${pool_name(i)}_ADDRESS_v11a`);
  });
  restore_env("CAPS_ADDRESS_v10c");
  cache_clear();
  discover_clear();
