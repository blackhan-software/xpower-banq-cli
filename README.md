...
💸 Supply `APOW` tokens: into `pool=2`

```sh
banq supply 1.0 APOW [--pool=2] [-Y|--broadcast]
```

💳 Borrow `XPOW` tokens: from `pool=2`

```sh
banq borrow 1.0 XPOW [--pool=2] [-Y|--broadcast]
```

🤑 Settle `XPOW` tokens: into `pool=2`

```sh
banq settle 1.0 XPOW [--pool=2] [-Y|--broadcast]
```

💰 Redeem `APOW` tokens: from `pool=2`

```sh
banq redeem 1.0 APOW [--pool=2] [-Y|--broadcast]
```

### User Management

🏥 Health of `$USER`'s positions: in `pool=2`

```sh
banq health-of $USER [--pool=2] [-Y|--broadcast] # USER=0x..
```

💦 Liquidate `$USER`'s positions: in `pool=2`

```sh
banq liquidate $USER [--pool=2] [-Y|--broadcast] # USER=0x..
```
...
📈 Supply and borrow rate(s) of `XPOW`: in `pool=2` at (±) index

```sh
banq rates-of XPOW [--pool=2] [-@|--at=now] [-Y|--broadcast]
```

📉 Supply and borrow rate(s) of `APOW`: in `pool=2` incl. history

```sh
banq rates-of APOW [--pool=2] [-@|--at=all] [-Y|--broadcast]
```
