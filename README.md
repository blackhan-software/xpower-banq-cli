...
```sh
export CONTRACT_RUN=v10b # default; v10b, v10c, v11a.
```
...
Config lives in YAML (`yml/banq.mainnet.yaml` / `yml/banq.testnet.yaml`, plus gitignored `yml/*.local.yaml` overrides). `cfg/generate.ts` flattens it into `.env/banq.{net}.env`, consumed via `--env`.
...
npm run banq
# or:
deno run -A cfg/generate.ts --network=mainnet && deno run -A --env=.env/banq.mainnet.env ./cli/banq/banq-main.ts
```
