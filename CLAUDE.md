...
deno test -A path/to/specific-file.test.ts      # run single test file
```
...
Individual build tasks are available in `deno.json` for specific platforms (Linux x86_64/aarch64, macOS x86_64/aarch64, Windows x86_64) and networks (mainnet/testnet).
...
- **`env/`**: Environment-based configuration
  ...
  - `find-by.ts` - Helper to look up addresses by identifier (prefix/suffix parsing)
...
The versioning system allows multiple contract versions to coexist. The `CONTRACT_RUN` env var or `--contract-run` flag selects which version to use.
...
- Run with `deno test -A` (the `--env=.env` flag is implicit in the project setup)
