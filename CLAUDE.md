...
Commands are dispatched by name or short alias: `pass`/`pa`, `supply`/`su`, `borrow`/`bo`, `settle`/`se`, `redeem`/`re`, `liquidate`/`li`, `refresh`/`rf`, `retwap`/`rt`, `reindex`/`ri`, `health-of`/`ho`, `rates-of`/`ro`, `xpow-init`/`xi`, `xpow-mine`/`xm`, `xpow-mint`/`xt`, `apow-claim`/`ac`, `apow-claim-batch`/`acb`, `acma` (no short alias).

The `acma` command has its own internal subcommand router in `cmd/banq/acma.ts` — subcommands `show` (default), `roles`, `members`, `targets`, `hierarchy`, `delays`, `logs` — backed by the `acma-*.ts` helpers in `cmd/banq/tool/`.
