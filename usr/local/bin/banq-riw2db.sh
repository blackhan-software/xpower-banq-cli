...
DB_PATH="${1-/var/lib/banq/logs.db}"
DB_PAGE="${2:-1}" # batch size
...
-- Text of JSON logs
--
CREATE TABLE IF NOT EXISTS raw_logs (json TEXT NOT NULL);
--
-- View of JSON logs
--
CREATE VIEW IF NOT EXISTS riw_logs AS
  SELECT
  ...
  FROM raw_logs;
--
-- Indexes (faster queries by id/time):
--
CREATE INDEX IF NOT EXISTS idx_id
  ON raw_logs (json_extract(json,'$.id'));
CREATE INDEX IF NOT EXISTS idx_block_number
  ON raw_logs (json_extract(json,'$.log.blockNumber'));
...
while IFS= read -r line; do
  ...
  printf "INSERT INTO raw_logs(json) VALUES('%s');\n" "$esc" >&3 2>/dev/null || break
  n=$((n+1))
  if (( n % DB_PAGE == 0 )); then
    printf 'COMMIT; BEGIN;\n' >&3 2>/dev/null || break
  fi
