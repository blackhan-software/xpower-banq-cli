...
import type { LedgerTransport, LedgerTransportArg } from "./hw-ledger-types.ts";
...
export class LedgerSigner extends AbstractSigner {
  ...
  constructor(
    ...
    ///
    if ("create" in transport && typeof transport.create === "function") {
      transport = transport.create();
    }
    this._transport = Promise.resolve(transport) as Promise<LedgerTransport>;
...
async function sign_eip712(
  ...
  try {
    return await eth.signEIP712Message(path, payload);
    // deno-lint-ignore no-explicit-any
  } catch (e: any) {
