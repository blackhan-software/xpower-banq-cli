 ...
 * errors and continue producing output.
 */
export async function command(args: BanqArgs): Promise<CommandResult> {
  ...
  if (!args.broadcast) {
    return [[token_arg.symbol, pow_level, nonce_length, x(to)], [DRY_RUN]];
  }
...
async function blockHashOf(
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    const interval = await xpow.currentInterval().catch((e: unknown) => {
      console.error(e);
      return null;
    });
    if (!interval || interval != Miner.interval() || !BigInt(interval)) {
      continue;
    }
    // get block-hash for current-interval
    const block_hash = await xpow.blockHashOf(interval).catch(
      (e: unknown) => {
        console.error(e);
        return null;
      },
  ...
  return { block_hash: undefined, interval: undefined };
}
