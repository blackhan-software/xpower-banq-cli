...
export async function command(args: BanqArgs): Promise<CommandResult> {
  ...
  try {
    const [sp_address, bp_address, vault] = await Promise.all([
      POOL.supplyOf(x(token)),
      POOL.borrowOf(x(token)),
      POOL.vaultOf(x(token)),
    ]);
    const S_POS = new ethers.Contract(sp_address, POSITION_ABI, signer);
    const B_POS = new ethers.Contract(bp_address, POSITION_ABI, signer);
    ...
    const [info, utils, sp, bp] = await Promise.all([
