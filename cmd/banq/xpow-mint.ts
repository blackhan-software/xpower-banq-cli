...
function gas_options(args: BanqArgs): GasOptions {
  const { max_priority_fee_per_gas } = opt_max_priority_fee_per_gas(args);
  const { max_fee_per_gas } = opt_max_fee_per_gas(args);
  const { gas_limit } = opt_gas_limit(args);
  return {
