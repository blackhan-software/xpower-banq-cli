...
import { discover } from "../../env/registry.ts";
...
export async function command(args: BanqArgs): Promise<CommandResult> {
  ...
  if (args.acma === undefined && args.pool === undefined) {
    throw new ArgumentError(
      "acma: --pool is required (or --acma to specify the access-manager address)",
    );
  }
  if (!args.broadcast) {
    return [[sub], [DRY_RUN]];
  ...
  await discover(provider, contract_run); // populate reverse labels for display
  const acma_addr = await resolve_acma_address(args, provider);
...
async function scan_events(sps: ScanParams): Promise<AcmaEvent[]> {
  const { provider, acma_hex, chain_id, to_block, max_blocks, on_block, run } =
    sps;
...
async function resolve_acma_address(
  args: BanqArgs,
  ...
  if (args.acma) {
    ...
    throw new ArgumentError(`invalid acma address: ${args.acma}`);
  }
  // fallback: the pool's authority is the access manager
  const { address: pool } = opt_pool(args);
  const { contract_run } = opt_contract_run(args);
  const POOL = new ethers.Contract(x(pool), pool_abi(contract_run), provider);
  return BigInt(await POOL.authority());
}
