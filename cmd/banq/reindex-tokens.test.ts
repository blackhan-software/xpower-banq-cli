...
Deno.test(
  "banq -p P000 -M supply -w 100@all [reindex, APOW]",
  OPTS,
  async () => {
    const args = {
      pool: "P000",
    ...
    const call = ["reindex", ["sAPOW", JSON.stringify([100, Infinity])], [
      false,
    ]];
    const next = await cli_next(args as BanqArgs);
    assertEquals(next.value, call);
...
Deno.test("banq -p P000 -M borrow [reindex, XYZT]", OPTS, async () => {
  const args = {
    pool: "P000",
    mode: "borrow",
    rest: ["reindex", "XYZT"],
  };
  const call = ["reindex", ["bXYZT", false], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
