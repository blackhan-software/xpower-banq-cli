...
Deno.test("banq -p P000 -M supply [reindex, APOW]", OPTS, async () => {
  ...
  const call = ["reindex", ["sAPOW"], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
Deno.test(
  `banq -p P000 -M borrow [reindex, ${abbressOf(APOW)}]`,
  OPTS,
  async () => {
    const args = {
      pool: "P000",
    ...
    const call = ["reindex", ["bAPOW"], [false]];
    const next = await cli_next(args as BanqArgs);
    assertEquals(next.value, call);
  },
);
Deno.test("banq -p P000 -M supply [reindex, XPOW]", OPTS, async () => {
  ...
  const call = ["reindex", ["sXPOW"], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
Deno.test(
  `banq -p P000 -M borrow [reindex, ${abbressOf(XPOW)}]`,
  OPTS,
  async () => {
    const args = {
      pool: P000,
    ...
    const call = ["reindex", ["bXPOW"], [false]];
    const next = await cli_next(args as BanqArgs);
    assertEquals(next.value, call);
  },
);
