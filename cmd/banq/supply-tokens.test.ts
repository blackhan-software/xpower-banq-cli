 ...
 */
Deno.test("banq -p P000 [supply, 0.0, APOW]", OPTS, async () => {
  const args = { pool: "P000", rest: ["supply", 0.0, "APOW"] };
  const call = ["supply", [0.0, "APOW"], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
Deno.test("banq -p P000 [supply, 0.0000000001, APOW]", OPTS, async () => {
  const args = { pool: "P000", rest: ["supply", 0.0000000001, "APOW"] };
  const call = ["supply", [0.0000000001, "APOW"], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
Deno.test("banq -p P000 [supply, 1e18, APOW]", OPTS, async () => {
  const args = { pool: "P000", rest: ["supply", 1e18, "APOW"] };
  const call = ["supply", [1e18, "APOW"], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
Deno.test(
  "banq -p P000 [supply, 1.123456789012345678, APOW]",
  OPTS,
  async () => {
    const args = {
      pool: "P000",
      rest: ["supply", 1.123456789012345678, "APOW"],
    };
    const call = ["supply", [1.123456789012345678, "APOW"], [false]];
    const next = await cli_next(args as BanqArgs);
    assertEquals(next.value, call);
  },
);
...
Deno.test("banq -p P000 [supply, 1.0, XYZT]", OPTS, async () => {
  const args = { pool: "P000", rest: ["supply", 1.0, "XYZT"] };
  const call = ["supply", [1.0, "XYZT"], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
