...
/**
 * Split a role label into its base and a trailing `_P<digits>` pool suffix.
 * `SUPPLY_SET_TARGET_ROLE_P000` → { base: "SUPPLY_SET_TARGET_ROLE", pool: "_P000" }.
 */
function split_label_pool(label: string): { base: string; pool: string } {
  const m = /^(.*)(_P\d+)$/.exec(label);
  if (!m) {
    return { base: label, pool: "" };
  }
  return { base: m[1], pool: m[2] };
}
...
function apply_labeled(
  ...
  const role = ensure_role(roles, ev.roleId!);
  role.label = ev.label!;
  // infer tier from label suffix, ignoring a trailing _P<pool> suffix
  const { base } = split_label_pool(ev.label!);
  if (base.endsWith("_GUARD_ROLE")) {
    role.tier = "grd";
  } else if (base.endsWith("_ADMIN_ROLE")) {
    role.tier = "adm";
  } else if (base.endsWith("_ROLE")) {
    role.tier = "exe";
  }
}
...
export function role_name(role: RoleState): string {
  ...
  // strip _GUARD_ROLE, _ADMIN_ROLE, _ROLE suffix, keeping a trailing _P<pool>
  const { base, pool } = split_label_pool(label);
  if (base.endsWith("_GUARD_ROLE")) {
    return base.slice(0, -"_GUARD_ROLE".length) + pool;
  }
  if (base.endsWith("_ADMIN_ROLE")) {
    return base.slice(0, -"_ADMIN_ROLE".length) + pool;
  }
  if (base.endsWith("_ROLE")) {
    return base.slice(0, -"_ROLE".length) + pool;
  }
  return base + pool;
}
...
/**
 * Map each target contract to its display prefix, derived from the role
 * labels of its protected functions (first `_`-token, e.g. `POLS_FETCH0_ROLE`
 * → `POLS`). Lets target contracts like POLS/VAULT be labeled in matrices
 * when they appear as role members.
 */
export function target_names(state: AcmaState): Map<bigint, string> {
  const names = new Map<bigint, string>();
  for (const target of state.targets.values()) {
    if (target.functions.size === 0) continue;
    for (const roleId of target.functions.values()) {
      const role = state.roles.get(roleId);
      if (!role) continue;
      const rn = role_name(role);
      const idx = rn.indexOf("_");
      const prefix = idx < 0 ? rn : rn.slice(0, idx);
      if (prefix !== "" && prefix !== "ADMIN" && prefix !== "PUBLIC") {
        names.set(target.address, prefix);
        break;
      }
    }
  }
