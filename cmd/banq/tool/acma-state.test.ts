...
/**
 * @group positive tests — reconstruct labeled pool suffix
 */
Deno.test("acma-state: RoleLabel tiers ignore trailing pool suffix", () => {
  const events: AcmaEvent[] = [{
    block: 100,
    logIndex: 0,
    txHash: "0x1",
    kind: "labeled",
    roleId: 10n,
    label: "SUPPLY_SET_TARGET_ROLE_P000",
  }, {
    block: 100,
    logIndex: 1,
    txHash: "0x1",
    kind: "labeled",
    roleId: 11n,
    label: "SUPPLY_SET_TARGET_ADMIN_ROLE_P000",
  }, {
    block: 100,
    logIndex: 2,
    txHash: "0x1",
    kind: "labeled",
    roleId: 12n,
    label: "SUPPLY_SET_TARGET_GUARD_ROLE_P000",
  }];
  const state = reconstruct(events);
  assertEquals(state.roles.get(10n)!.tier, "exe");
  assertEquals(state.roles.get(11n)!.tier, "adm");
...
/**
 * @group positive tests — role_name stripping with pool suffix
 */
Deno.test("acma-state: role_name strips suffixes before pool suffix", () => {
  const state = reconstruct([{
    block: 1,
    logIndex: 0,
    txHash: "0x1",
    kind: "labeled",
    roleId: 10n,
    label: "POOL_CAP_SUPPLY_ROLE_P007",
  }, {
    block: 1,
    logIndex: 1,
    txHash: "0x1",
    kind: "labeled",
    roleId: 11n,
    label: "POOL_CAP_SUPPLY_ADMIN_ROLE_P007",
  }, {
    block: 1,
    logIndex: 2,
    txHash: "0x1",
    kind: "labeled",
    roleId: 12n,
    label: "POOL_CAP_SUPPLY_GUARD_ROLE_P007",
  }]);
  assertEquals(role_name(state.roles.get(10n)!), "POOL_CAP_SUPPLY_P007");
  assertEquals(role_name(state.roles.get(11n)!), "POOL_CAP_SUPPLY_P007");
  assertEquals(role_name(state.roles.get(12n)!), "POOL_CAP_SUPPLY_P007");
});
...
/**
 * @group positive tests — target_names
 */
Deno.test("acma-state: target_names derives prefixes from role labels", () => {
  ...
  assertEquals(names.get(pols_addr), "POLS");
  assertEquals(names.get(vault_addr), "VAULT");
});

/**
 * @group positive tests — target_names pool suffix
 */
Deno.test("acma-state: target_names strips pool suffix", () => {
  const pool_addr = 0x1000000000000000000000000000000000000003n;
  const events: AcmaEvent[] = [
    {
      block: 100,
      logIndex: 0,
      ...
      kind: "labeled",
      roleId: 1n,
      label: "POOL_CAP_BORROW_ROLE_P007",
    },
    {
      block: 101,
      logIndex: 0,
  ...
  const state = reconstruct(events);
  const names = target_names(state);
  assertEquals(names.get(pool_addr), "POOL");
});
