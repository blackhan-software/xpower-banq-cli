...
import { group_roles, pool_display } from "./acma-group.ts";
...
export function acma_show(
  ...
  const names = target_names(state);
  const addrs = sorted_by_label(all_addresses(state), names);
  const rows_list = group_roles(
    sorted_roles(state).filter((r) => r.members.size > 0),
  );
  // column headers: named columns render label + hex prefix adjacent;
  // unnamed columns repeat the address as their label
  const cell_w = 3;
  const columns = addrs.map((a) => {
    const name = prefix_by(a) ?? names.get(a);
    const hex = hex_prefix(a);
    return name !== undefined ? [name.slice(0, 4), hex] : [hex, hex];
  });
  // vertical headers
  const v_rows = vertical_headers_pairs(columns, cell_w);
  // role columns: first prefix, middle parts, pool range, tier
  const first_w = Math.max(4, ...rows_list.map((r) => r.first.length));
  const middle_w = Math.max(0, ...rows_list.map((r) => r.middle.length));
  const pool_w = Math.max(
    4,
    ...rows_list.map((r) => pool_display(r.pool).length),
  );
  const tier_w = 4;
  // build header rows
  const prefix_w = first_w + 1 + middle_w + 1 + pool_w + 1 + tier_w + 1;
  const header_rows = v_rows.map((row) => " ".repeat(prefix_w) + row);
  // dot header row
  const dots = addrs.map(() =>
    "·".padStart(Math.ceil(cell_w / 2)).padEnd(cell_w)
  ).join("");
  const dot_row = pad_right("Role", first_w + 1 + middle_w) + " " +
    pad_left("pool", pool_w) + " " + pad_left("tier", tier_w) + " " + dots;
  // separator
  const sep_w = prefix_w + addrs.length * cell_w;
  const sep = separator(sep_w);
  // group by tier
  const rows: string[] = [...header_rows, dot_row, sep];
  for (const [i, row] of rows_list.entries()) {
    if (i > 0 && rows_list[i - 1].tier !== row.tier) {
      rows.push(sep);
    }
    const first = pad_right(row.first, first_w);
    const middle = pad_right(row.middle, middle_w);
    const pool = pad_left(pool_display(row.pool), pool_w);
    const tier = pad_left(row.tier, tier_w);
    const cells = addrs.map((addr) => {
      const member = row.role.members.get(addr);
      const mark = pending_mark(member, row.role.grantDelay);
      return matrix_cell(mark, cell_w);
    }).join("");
    rows.push(`${first} ${middle} ${pool} ${tier} ${cells}`);
  }
  // footer
  ...
  if (options.addresses) {
    const addr_rows = addrs.map((a) => {
      ...
      return `${legend_label(a, names)}  ${count} role${
        count !== 1 ? "s" : ""
      }`;
    });
...
export function acma_members(
  ...
  const delay_w = rows_list.reduce(
    (max, r) =>
      Math.max(
        max,
        ...[...r.role.members.values()].map((m) =>
          format_delay(m.execDelay).length
        ),
      ),
    1,
  );
  const cell_w = Math.max(3, delay_w + 1);
  // column headers: named columns render label + hex prefix adjacent;
  // unnamed columns repeat the address as their label
  const columns = addrs.map((a) => {
    const name = prefix_by(a) ?? names.get(a);
  ...
  for (const [i, row] of rows_list.entries()) {
    ...
    const tier = pad_left(row.tier, tier_w);
    const cells = addrs.map((addr) => {
      const m = row.role.members.get(addr);
      const val = m ? format_delay(m.execDelay) : "·";
      return val.padStart(Math.ceil(cell_w / 2)).padEnd(cell_w);
    }).join("");
    rows.push(`${first} ${middle} ${pool} ${tier} ${cells}`);
  }
  const sections: FrameSection[] = [{
  ...
  if (options.addresses) {
    const addr_rows = addrs.map((a) => legend_label(a, names));
    sections.push({ rows: addr_rows });
  }
  return frame(sections);
}
...
export function acma_targets(state: AcmaState): string {
  ...
  for (const target of targets) {
    const lbl = legend_label(target.address, names);
...
export function acma_delays(
  ...
  const names = target_names(state);
  const name_of = (addr: bigint) => prefix_by(addr) ?? names.get(addr);
  const targets = options.addresses ? all_targets : all_targets
    .filter((t) => name_of(t.address) !== undefined)
    .sort((a, b) => {
      const la = name_of(a.address)!;
      const lb = name_of(b.address)!;
  ...
  for (const tgt of targets) {
    const lbl = legend_label(tgt.address, names);
    tgt_rows.push(
...
export function acma_logs(
  ...
  for (const ev of events) {
    ...
    const detail = format_detail(state, ev, options, names);
...
function format_detail(
  ...
  switch (ev.kind) {
    case "granted": {
      ...
      const acct = label_of(ev.account!, names);
    ...
    case "fn-role": {
      const tgt = label_of(ev.target!, names);
      const role = state.roles.get(ev.roleId!);
      const rn = role ? role_name(role) : ev.roleId!.toString(16);
    ...
    case "op·sched": {
      ...
      const caller = label_of(ev.caller!, names);
      const tgt = label_of(ev.target!, names);
