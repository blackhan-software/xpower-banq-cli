...
Deno.test("acma-format: vertical_headers_pairs adjacent", () => {
  ...
  // labels render adjacent with no inner spacing; label (first group) is bright white
  assertEquals(rows[0], `${BW}N${RESET}9`);
  assertEquals(rows[1], `${BW}A${RESET}F`);
  assertEquals(rows[2], `${BW}M${RESET}3`);
  assertEquals(rows[3], `${BW}E${RESET}A`);
});
...
Deno.test("acma-format: vertical_headers_pairs single left-aligned", () => {
  ...
  assertEquals(rows.length, 4);
  assertEquals(rows[0], `${BW}A${RESET}  `);
  assertEquals(rows[3], `${BW}D${RESET}  `);
});

/**
 * @group positive tests — matrix_cell
 */
Deno.test("acma-format: matrix_cell non-member stays plain", () => {
  assertEquals(matrix_cell("·", 3), " · ");
});
Deno.test("acma-format: matrix_cell active grant bright white", () => {
  assertEquals(matrix_cell("+", 3), ` ${BW}+${RESET} `);
});
Deno.test("acma-format: matrix_cell pending circle bright white", () => {
  assertEquals(matrix_cell("○", 3), ` ${BW}○${RESET} `);
});
