...
Deno.test("acma-show: acma_show renders frame", OPTS, () => {
  ...
  assertEquals(output.includes("POOL"), true);
  assertEquals(output.includes("CAP_SUPPLY"), true);
  assertEquals(output.includes("RETWAP"), true);
  assertEquals(output.includes("pool"), true);
  assertEquals(output.includes("tier"), true);
});
...
/**
 * @group positive tests — acma_show pool column
 */
Deno.test("acma-show: acma_show labels supra-pool and pool roles", OPTS, () => {
  const events: AcmaEvent[] = [
    ...
    },
    {
      ...
      kind: "labeled",
      roleId: 11n,
      label: "POOL_TMP_SUPPLY_ROLE_P000",
    },
    {
      block: 100,
      logIndex: 2,
      ...
      kind: "labeled",
      roleId: 12n,
      label: "POOL_TMP_SUPPLY_ROLE_P001",
    },
    {
      block: 101,
      logIndex: 0,
  ...
  const output = acma_show(state, 12345678, { addresses: false });
  assertEquals(output.includes("any"), true);
  assertEquals(output.includes("0:1"), true);
  assertEquals(output.includes("FETCH0"), false);
});
...
/**
 * @group positive tests — target contract member labeled
 */
Deno.test("acma-show: target contract members get named columns", OPTS, () => {
  const pols_addr = 0x1000000000000000000000000000000000000001n;
  const events: AcmaEvent[] = [
    {
      block: 1,
      logIndex: 0,
  ...
  const state = reconstruct(events);
  const output = acma_show(state, 12345678, { addresses: false });
  assertEquals(output.includes("POLS"), true);
  // named columns render the hex prefix adjacent to the name (label-attack review);
  // the label char is bright white
  assertEquals(output.includes("\x1b[1;37mP\x1b[0m1"), true);
  assertEquals(output.includes("\x1b[1;37mS\x1b[0m0"), true);
});

/**
 * @group positive tests — delays includes target-named contracts
 */
Deno.test("acma-show: acma_delays lists target-named contracts", OPTS, () => {
  const pols_addr = 0x1000000000000000000000000000000000000001n;
  const vault_addr = 0x1000000000000000000000000000000000000002n;
  const events: AcmaEvent[] = [
    {
      block: 1,
      logIndex: 0,
  ...
  const state = reconstruct(events);
  const output = acma_delays(state, { addresses: false });
  // legend entries `SHORT: ABBREV (NAME)` only appear in the target list
  assertEquals(output.includes("POLS: 0x1000"), true);
  assertEquals(output.includes("VAUL: 0x1000"), true);
  assertEquals(output.includes("VAULT: 0x1000"), false);
});
