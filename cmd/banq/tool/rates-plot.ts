...
export async function plot(
  ...
  if (do_plot(subplot, UT_WORDS)) {
    ...
    console.log(
      chart.plot(data[0], { colors: ["lightblue"], ...conf(UT_WORDS) }),
    );
  }
  //
  // plot supply rates
  ...
  if (do_plot(subplot, SU_WORDS)) {
    ...
    console.log(
      chart.plot(data[1], { colors: ["lightgreen"], ...conf(SU_WORDS) }),
    );
  }
  //
  // plot borrow rates
  ...
  if (do_plot(subplot, BO_WORDS)) {
    ...
    console.log(
      chart.plot(data[2], { colors: ["lightred"], ...conf(BO_WORDS) }),
    );
  }
  //
  // plot timestamps
  ...
  if (do_plot(subplot, TS_WORDS)) {
    ...
    const count = horz_offset + usbt.length - 2 * lhs.length - 5;
...
function if_log(
  ...
  if (typeof subplots === "string") {
    ...
    for (const subplot of subplots.split(",")) {
      ...
      if (
        subplot.match(/-log$/i) &&
          is_prefix(plot, words) || eq_i(subplot, "log")
      ) {
        return true;
      }
    }
