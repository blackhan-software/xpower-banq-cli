...
export function plot(
  ...
  if (use_plot(options, UT_WORDS)) {
    ...
    const title = `> ${options.symbol} Pool [U]tilization Rates`;
    const space = " ".repeat(terminal_cols - title.length - 4);
    console.log([
      `╔${"═".repeat(terminal_cols - 2)}╗`,
  ...
  if (use_plot(options, SU_WORDS)) {
    ...
    const title = `> ${options.symbol} [S]upply Interest Rates`;
    const space = " ".repeat(terminal_cols - title.length - 4);
    console.log([
      `╠${"═".repeat(terminal_cols - 2)}╣`,
      `║ ${chart.colored(title, "bold")}${space} ║`,
  ...
  if (use_plot(options, BO_WORDS)) {
    ...
    const title = `> ${options.symbol} [B]orrow Interest Rates`;
    const space = " ".repeat(terminal_cols - title.length - 4);
    console.log([
      `╠${"═".repeat(terminal_cols - 2)}╣`,
      `║ ${chart.colored(title, "bold")}${space} ║`,
  ...
  if (use_plot(options, TS_WORDS)) {
    ...
    const count = offset + usbt.length - 2 * lhs.length - 4;
    const title = `${lhs}|${":".repeat(count)}|${rhs}`;
    const space = " ".repeat(terminal_cols - title.length - 4);
    console.log([
      `╠${"═".repeat(terminal_cols - 2)}╣`,
      `║ ${chart.colored(title)}${space} ║`,
      `╚${"═".repeat(terminal_cols - 2)}╝`,
    ].join("\n"));
  }
}
