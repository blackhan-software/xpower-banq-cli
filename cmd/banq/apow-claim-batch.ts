...
export async function command(args: BanqArgs): Promise<CommandResult> {
  ...
  const token_arg = arg_token_by(args, args.rest, "APOW");
  const nft_ids = opt_nft_ids(args);
  assert(nft_ids, `invalid nft-id`);
  const { address: to } = opt_to(args);
  if (!args.broadcast) {
    return [[token_arg.symbol, ...nft_ids, x(to)], [DRY_RUN]];
  }
  ...
  const [pool, amounts] = await Promise.all([
    ...
    mty.mintableBatch(x(to_address), nft_ids) as Promise<bigint[]>,
  ]);
...
async function resolve_nonce(
  ...
  const [_, nonce] = await pow(
    supplyData(to_address, apow_target, amount),
    difficulty,
    {
