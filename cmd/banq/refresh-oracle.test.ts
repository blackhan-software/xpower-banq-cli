...
Deno.test("banq -oT000 [refresh, XYZT]", OPTS, () => {
...
Deno.test("banq -oT000 [refresh, XPOW]", OPTS, () => {
...
Deno.test("banq -oT000 [refresh, XPOW, XYZT]", OPTS, async () => {
  const args = { oracle: "T000", rest: ["refresh", "XPOW", "XYZT"] };
  const call = ["refresh", ["XPOW", "XYZT"], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
