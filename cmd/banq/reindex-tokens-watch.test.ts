...
Deno.test(
  "banq -p P000 -M borrow [reindex, XYZT, --watch=100]",
  OPTS,
  async () => {
    const args = {
      pool: "P000",
    ...
    const call = ["reindex", ["bXYZT", "[100]"], [false]];
    const next = await cli_next(args as BanqArgs);
    assertEquals(next.value, call);
  },
);
...
Deno.test("banq -p P000 -M borrow [reindex, XYZT, --watch]", OPTS, async () => {
  const args = {
    pool: "P000",
    mode: "borrow",
    watch: "",
  ...
  const call = ["reindex", ["bXYZT", true], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
