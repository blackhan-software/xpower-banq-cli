...
Deno.test("banq [xpow-mine, XPOW, to=0n, pow-level=1, nonce-length=8]", OPTS, async () => {
  const args = {
    to: 0n, pow_level: 1, nonce_length: 8, rest: ["xpow-mine", "XPOW"]
  };
  const call = ["xpow-mine", ["XPOW", 1, 8, x(0n)], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
Deno.test("banq [xpow-mine, to=1n, pow-level=64, nonce-length=16]", OPTS, async () => {
  const args = {
    to: 1n, pow_level: 64, nonce_length: 16, rest: ["xpow-mine", "XPOW"]
  };
  const call = ["xpow-mine", ["XPOW", 64, 16, x(1n)], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
/**
