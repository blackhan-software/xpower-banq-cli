...
Deno.test(
  "banq -oT000 -w 100@all [retwap, XPOW, APOW]",
  OPTS,
  async () => {
    const args = {
      oracle: "T000",
    ...
    const call = [
      "retwap",
      ["XPOW", "APOW", JSON.stringify([100, Infinity])],
      [false],
    ];
    const next = await cli_next(args as BanqArgs);
    assertEquals(next.value, call);
...
Deno.test("banq -oT000 [retwap, XYZT]", OPTS, () => {
...
Deno.test("banq -oT000 [retwap, XPOW]", OPTS, () => {
...
Deno.test("banq -oT000 [retwap, XPOW, XYZT]", OPTS, async () => {
  const args = { oracle: "T000", rest: ["retwap", "XPOW", "XYZT"] };
  const call = ["retwap", ["XPOW", "XYZT"], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
