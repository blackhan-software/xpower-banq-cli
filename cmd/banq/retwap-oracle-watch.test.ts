...
Deno.test("banq -oT000 [retwap, XYZT, APOW, --watch=100]", OPTS, async () => {
  const args = {
    oracle: "T000",
    watch: "100",
    rest: ["retwap", "XYZT", "APOW"],
  };
  const call = ["retwap", ["XYZT", "APOW", "[100]"], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
...
Deno.test("banq -oT000 [retwap, APOW, XYZT, --watch=100]", OPTS, async () => {
  const args = {
    oracle: "T000",
    watch: "100",
    rest: ["retwap", "APOW", "XYZT"],
  };
  const call = ["retwap", ["APOW", "XYZT", "[100]"], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
...
Deno.test("banq -oT000 [retwap, XYZT, APOW, --watch]", OPTS, async () => {
  const args = {
    oracle: "T000",
    watch: "",
    rest: ["retwap", "XYZT", "APOW"],
  };
  const call = ["retwap", ["XYZT", "APOW", true], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
...
Deno.test("banq -oT000 [retwap, APOW, XYZT, --watch]", OPTS, async () => {
  const args = {
    oracle: "T000",
    watch: "",
    rest: ["retwap", "APOW", "XYZT"],
  };
  const call = ["retwap", ["APOW", "XYZT", true], [false]];
  const next = await cli_next(args as BanqArgs);
  assertEquals(next.value, call);
});
