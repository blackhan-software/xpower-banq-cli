...
/**
 * track-position $TOKEN --mode={supply,borrow} [--watch|--watch=<RANGE>] [--options]
 */
...
async function execute_watch_all(
  ...
  if (!args.broadcast) {
    return [[my(token_arg.symbol, mode), JSON.stringify([dlt, Infinity])], [
      DRY_RUN,
    ]];
  }
  ...
  ///
  /// track-position $TOKEN --watch=DLT@ALL
  ///
  ...
  const decimals = await POSITION.decimals();
  assert(decimals > 0n, "invalid decimals");
  ...
  for (
    ...
    for (const ev_log of event_logs as ethers.EventLog[]) {
      const [from, to] = ev_log.args ?? [x(0n), x(0n)];
      console.log(JSON.stringify({
        id: uuidv5(JSON.stringify(ev_log)),
        filter: "Transfer",
        from: x(from),
        to: x(to),
        value: ev_log.args?.[2] ?? 0n,
        unit: 10n ** decimals,
        symbol,
        token: x(token),
        mode,
  ...
  return [
    [my(symbol, mode), JSON.stringify([dlt, Infinity])],
    [BROADCAST],
  ];
}
async function execute_watch_in(
  ...
  const { mode } = opt_mode(args);
  if (!args.broadcast) {
    return [[my(token_arg.symbol, mode), JSON.stringify(range)], [DRY_RUN]];
  }
  const { contract_run: run } = opt_contract_run(args);
  const { signer } = await wallet(args, true);
  const reg = await discover(signer.provider!, run);
  const { address: token, symbol } = resolve_arg_token(token_arg, reg);
  ///
  /// track-position $TOKEN --watch=DLT or --watch=DLT@IDX
  ///
  const POOL = new ethers.Contract(x(pool), pool_abi(run), signer);
  ...
  return [
    [my(symbol, mode), JSON.stringify(range)],
    [BROADCAST],
  ];
}
async function execute_watch(
  ...
  const { mode } = opt_mode(args);
  if (!args.broadcast) {
    return [[my(token_arg.symbol, mode), true], [DRY_RUN]];
  }
  const { contract_run: run } = opt_contract_run(args);
  const { signer } = await wallet(args, true);
  const reg = await discover(signer.provider!, run);
...
async function execute(
  ...
  const { mode } = opt_mode(args);
  if (!args.broadcast) {
    return [[my(token_arg.symbol, mode), false], [DRY_RUN]];
  }
...
async function resolve_position(
  ...
  try {
    ...
    return [error_args, [(e as Error).message]];
  }
}
