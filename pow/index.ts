...
export async function pow(
  ...
  timeout = 60_000,
): Promise<[data: string, nonce: bigint]> {
  ...
  return pow_base(hasher, Number(difficulty), bytes, timeout);
}
function pow_base(
  hasher: IHasher,
  zeros: number,
  bytes: Uint8Array,
  timeout: number,
): [data: string, nonce: bigint] {
  const deadline = Date.now() + timeout;
  const step = BigInt(4 ** zeros);
  for (let i = 0n; Date.now() < deadline; i++) {
    ...
    return ["0x" + hexlify(bytes).slice(106), nonce]; // 2 + 64 + 40
  }
  throw new Error("timeout: no valid nonce found");
}
function pow_wasm(
  hasher: IHasher,
  bytes: Uint8Array,
